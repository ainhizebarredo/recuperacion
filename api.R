...
#* @apiTitle Exportaciones e importaciones
...
importaciones<-datos%>%
  group_by(`territorio histórico`)%>%
  filter(flujo=='Importaciones' & mes==mes)%>%
  summarise(imp_total=sum(cantidad, na.rm = T))

exportaciones<-datos%>%
  group_by(`territorio histórico`)%>%
  filter(flujo=='Exportaciones', mes==mes)%>%
  summarise(imp_total=sum(cantidad, na.rm = T))
...
ejericio2<-ejercicio2%>%
  filter(mes.impo==mes_ & mes.ex==mes_)%>%
  arrange(desc(suma))%>%
  head(1)

print(paste('El país con mayor volumen de importaciones ha sido', ejericio2$territorio.impo[1], 'y en el que Euskadi más ha exportado ha sido', ejericio2$territorio.ex[1]))
}
